...
module.exports = {
	...
	do_insert: function(data, user, _callback) {
		if (data && data != []) {
			_collection('question').insertMany(data, function(err, obj) {
				let id = obj.insertedId;
				insert_question_into_exam(data.exam, id);
				_callback(err, func.questions_template({_id: id}))
			})
	...
	do_search: function(data, user, _callback) {
		if(!data.string) {
			_collection('question').find({creator: objectId(user._id)}).limit(5).toArray(_callback)
		} else {
